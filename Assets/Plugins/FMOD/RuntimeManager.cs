...
namespace FMODUnity
{
    ...
    public class RuntimeManager : MonoBehaviour
    {
        ...
        FMOD.RESULT Initialize()
        {
            ...
            FMOD.RESULT resTest = LowlevelSystem.setDSPBufferSize(128, 2);
#endif
#if UNITY_STANDALONE_OSX
            FMOD.RESULT resTest = LowlevelSystem.setDSPBufferSize(128, 8);
#endif
        ...
        void Update()
        {
            if (studioSystem.isValid())
            {
                ...
                for (int i = attachedInstances.Count - 1; i >= 0; i--)
                {
                    var currentAttachedInstance = attachedInstances[i];
                    
                    FMOD.Studio.PLAYBACK_STATE playbackState = FMOD.Studio.PLAYBACK_STATE.STOPPED;
                    
                    if (!currentAttachedInstance.instance.isValid() ||
                        currentAttachedInstance.transform == null // destroyed game object
                        )

                    {
                        RemoveAttachedInstance(currentAttachedInstance, i);
                        continue;
                    }
                    
                    currentAttachedInstance.instance.getPlaybackState(out playbackState);
                    ...
                    if (currentAttachedInstance.rigidBody)
                    {
                        currentAttachedInstance.instance.set3DAttributes(RuntimeUtils.To3DAttributes(currentAttachedInstance.transform, currentAttachedInstance.rigidBody));
                    }
                    else
                    {
                        currentAttachedInstance.instance.set3DAttributes(RuntimeUtils.To3DAttributes(currentAttachedInstance.transform, currentAttachedInstance.rigidBody2D));
                    }
                }
        ...
        void RemoveAttachedInstance(AttachedInstance currentAttachedInstance, int index)
        {
            var lastItemIndex = attachedInstances.Count - 1;
            var lastItem = attachedInstances[lastItemIndex];
            attachedInstances[lastItemIndex] = currentAttachedInstance;
            attachedInstances[index] = lastItem;
            attachedInstances.RemoveAt(lastItemIndex);
        }

        public static void AttachInstanceToGameObject(FMOD.Studio.EventInstance instance, Transform transform, Rigidbody rigidBody)
        {
            foreach (var attached in Instance.attachedInstances)
            {
                if (attached.transform == transform)
                {
                    return;
                }
            }
            ...
            {
                transform = transform, instance = instance, rigidBody = rigidBody
            };
        ...
        public static void AttachInstanceToGameObject(FMOD.Studio.EventInstance instance, Transform transform, Rigidbody2D rigidBody2D)
        {
            ...
            var attachedInstance = new AttachedInstance
            {
                transform = transform, instance = instance, rigidBody2D = rigidBody2D, rigidBody = null
            };
            Instance.attachedInstances.Add(attachedInstance);
        }
